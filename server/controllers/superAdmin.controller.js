...
static    async changeRoles(req,res) {
  ...
  User.findByIdAndUpdate(id, { roles: role}, (err, result) => {
      res.status(201).json({
          message: 'oooo, you have changed the role of the user',
          user: result
      });
  });
}
   ...
   static deleteUser = async (req,res)=>{
       ...
       const token = authorization.split(' ')[1];
       const decodedToken = jwtDecode(token);
       const{error}= validation.changeStatusValidations(userModel.deleteUser(req));
       ...
       if(decodedToken.user.roles !=='super-admin'){
           return res.status(401).json({
               message: 'you are not allowed to prform this action)-:',
               status: 401
           });
       }
   ...
   static DisableUsers = function (req,res,next){
    ...
    const token = authorization.split(' ')[1];
    const decodedToken = jwtDecode(token);
    const{error}= validation.changeStatusValidations(userModel.disactivateUser(req));
    ...
    if(decodedToken.user.roles !=='super-admin'){
        return res.status(401).json({
            message: 'Oooopsss unfortunately you are not allowed to prform this action)-:',
            status: 401
        });
    }
    ...
    {
        res.status(201).json({
            message:'the user has been disactivated'
        })
    });
}
}
